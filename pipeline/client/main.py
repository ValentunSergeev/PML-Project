...
def main():
  ...
  while True:
    ...
    # Send request with frame to server
    file = {'image': ('frame.jpg', pic, 'image/jpeg', {'Expires': '0'})}
    frame = {
      'detections': [
        {
          'startX' : 0.0,
    ...
    data = {'frame': json.dumps(frame)}

    requests.post(f"{HOST_URL}/frames/", files=file, data=data, timeout=5)
